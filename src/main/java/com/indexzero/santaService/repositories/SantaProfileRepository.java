...
public interface SantaProfileRepository extends JpaRepository<SantaProfile, Long> {
    ...
    @Query(value = "SELECT S.id, S.available, S.contact_email, S.info, S.price, S.profile_image, S.santa_profile_name FROM SANTA_PROFILE AS S LEFT JOIN USER_ACCOUNT AS U ON S.id = U.santa_profile_id WHERE S.available = TRUE AND U.postal_code = :postal_code"
    , nativeQuery = true)
